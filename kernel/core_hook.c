...
static int ksu_do_umount(struct path *path, int flags)
{
...
static int ksu_do_umount(struct path *path, int flags)
{
	...
	mnt_buffer = (char *)__getname();
	if (!mnt_buffer)
		return -ENOMEM;

	ret = dentry_path_raw(path->dentry, mnt_buffer, PATH_MAX);
	if (ret < 0) {
		__putname(mnt_buffer);
		return ret;
	}

	ret = ksu_sys_umount(mnt_buffer, flags);
	__putname(mnt_buffer);
	return ret;
}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	is_zygote_child = susfs_is_sid_equal(old->security, susfs_zygote_sid);
